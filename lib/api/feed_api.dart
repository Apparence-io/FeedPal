...
class ApiClient {
  ...
  Future<T> _request<T>(String endpoint, {Map<String, String>? headers, Map<String, String>? queryParams}) async {
    final url = Uri(
  ...
  Future<PaginatedResponse<Article>> listArticles(ListOptions options) async {
    try {
      ...
      _log('Error fetching articles with options ${options.toString()}: $error');
      rethrow;
    }
  }
  ...
  Future<Article?> getArticle({required String slug}) async {
    try {
      ...
      _log('Error fetching article with slug "$slug" and locale "${config.locale}": $error');
